...
def rerank(reranker: SharedBiEncoder, tokenizer:AutoTokenizer, question, corpus, retrieved_ids, max_length = 256, top_k=30):
    ...
    #                        disable=len(questions) < batch_size):
    #batch_retrieved_ids = retrieved_ids
    collated = tokenizer(
                texts,
                padding=True,
                truncation=True,
...
def main():
    ...
    for org_question in org_questions:
        ...
        rerank_ids, rerank_scores = rerank(reranker, reranker_tokenizer, question, corpus, retrieval_ids, args.cross_max_length, args.top_k)
        all_end = time.time() - start
        pre_times.append(pre_end)
        all_times.append(all_end)
        
    print("Avg. Preprocess Time:", sum(pre_times)/len(org_questions))
    print("Avg. All Time:", sum(all_times)/len(org_questions))
